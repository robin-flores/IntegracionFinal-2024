...
def show_prod():
    if 'username' in session:
        ...
        cur = db.connection.cursor()
        cur.execute('''SELECT p.*, c.cat_name as categ_name, c.cat_id as id_categ
        FROM products p
...
def add_product():
    if request.method == 'POST':
            ...
            cur = db.connection.cursor()
            cur.execute('INSERT INTO products(name_prod, desc_prod, price, category, stock) VALUES(%s,%s,%s,%s,%s,%s)',
...
def edit_product(id):
            ...
            cur = db.connection.cursor()
            cur.execute('UPDATE products SET name_prod = %s, desc_prod = %s, price = %s, category = %s,stock = %s WHERE prod_id = %s',
                (name, desc, price, category, stock, id))
            db.connection.commit()
            cur.close()
